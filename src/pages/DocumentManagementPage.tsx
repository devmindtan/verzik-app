...
export function DocumentManagementPage() {
  ...
  const [registerForm, setRegisterForm] = useState({
    fileHash: "",
    fileName: "",
    docType: "voucher" as Document["docType"],
    amount: "",
  });
  ...
  const [tenants, setTenants] = useState<
    Awaited<ReturnType<typeof blockchainService.getTenants>>
  >([]);
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        await blockchainService.registerDocument(
          selectedTenant,
          registerForm.fileHash,
          registerForm.fileName,
          registerForm.docType,
          session?.address || "0x0",
          registerForm.amount
  ...
  const handleCoSign = (e: React.FormEvent) => {
    ...
    setConfirm({
      ...
      variant: "info",
      onConfirm: async () => {
        await blockchainService.coSignDocument(
          selectedTenant,
  ...
  const handleRevoke = (doc: Document) => {
    setConfirm({
      ...
      variant: "danger",
      onConfirm: async () => {
        await blockchainService.revokeDocument(
          selectedTenant,
          doc.fileHash,
          session?.address || "0x0",
        );
  ...
  const filteredDocuments = documents.filter((doc) =>
    includesNormalized(
      `${doc.fileName} ${doc.fileHash} ${docTypeLabel[doc.docType] || doc.docType} ${doc.issuedBy} ${doc.coSigners.join(" ")}`,
      searchQuery,
    ),
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Register, co-sign, and manage blockchain-anchored documents
          </p>
        </div>
        {canRegister && (
          <button
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border dark:border-slate-700 p-4 flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border dark:border-slate-700 overflow-hidden">
        <div className="px-5 py-4 border-b bg-slate-50 dark:bg-slate-800/70 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-3 flex gap-2 text-xs flex-wrap">
              <span className="px-2 py-1 bg-green-100 text-green-800 rounded font-semibold">
                {documents.filter((d) => d.isValid && d.coSignQualified).length}{" "}
                Qualified
              </span>
              <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded font-semibold">
                {
                  documents.filter((d) => d.isValid && !d.coSignQualified)
                    .length
                }{" "}
                Pending
              </span>
              <span className="px-2 py-1 bg-red-100 text-red-800 rounded font-semibold">
                {documents.filter((d) => !d.isValid).length} Revoked
              </span>
            </div>
          </div>
          <div className="w-full lg:w-[22rem] lg:flex-shrink-0 lg:ml-auto">
            <SearchInput
              value={searchQuery}
              onChange={setSearchQuery}
              placeholder="Search by document, hash, type, issuer, or signer..."
            />
          </div>
        </div>
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-500">
                            {doc.issuedAt.toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                      <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded text-xs font-semibold">
                        <HighlightText
                          text={docTypeLabel[doc.docType] || doc.docType}
                          query={searchQuery}
                        />
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex flex-col gap-1">
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-semibold w-fit ${doc.coSignQualified ? "bg-teal-100 text-teal-800" : "bg-yellow-100 text-yellow-800"}`}
                        >
                          {doc.coSignQualified ? "Qualified" : "Pending"}
                        </span>
                        {!doc.isValid && (
                          <span className="px-2 py-0.5 rounded text-xs font-semibold bg-red-100 text-red-800 w-fit">
                            Revoked
                          </span>
                        )}
                      </div>
        ...
        >
          <form onSubmit={handleRegister} className="space-y-4">
            <Field label="File Hash">
              <input
                type="text"
                value={registerForm.fileHash}
                onChange={(e) =>
                  setRegisterForm({ ...registerForm, fileHash: e.target.value })
                }
            ...
            <Field label="File Name">
              <input
                ...
                onChange={(e) =>
                  setRegisterForm({ ...registerForm, fileName: e.target.value })
                }
                className="input"
                placeholder="e.g., Invoice_2024_01.pdf"
              />
            </Field>
            <Field label="Document Type">
              <AppSelect
                ...
                onChange={(value) =>
                  setRegisterForm({
                    ...registerForm,
                    docType: value as Document["docType"],
                  })
                }
                data={[
          ...
          <form onSubmit={handleCoSign} className="space-y-4">
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="text-xs text-gray-500 mt-1">
                <TruncatedHash value={selectedDoc.fileHash} />
              </p>
              <p className="text-xs text-gray-500 mt-1">
                Current signers: {selectedDoc.coSigners.length}
              </p>
            </div>
            <Field label="Co-Signer Address">
              <input
                type="text"
                value={coSignForm.signer}
                onChange={(e) => setCoSignForm({ signer: e.target.value })}
                className="input font-mono text-sm"
                placeholder="0x..."
              />
            </Field>
            <div className="flex gap-3 pt-2">
              ...
              <button
                type="submit"
                className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-semibold transition-colors"
              >
                Co-Sign
              </button>
            </div>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-sm text-gray-900">
                  <TruncatedHash value={selectedDoc.issuedBy} />
                </p>
              </div>
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-xs text-gray-500">Issued At</p>
                <p className="font-medium text-gray-900">
                  {selectedDoc.issuedAt.toLocaleString()}
                </p>
              </div>
            </div>
                  ...
                  {selectedDoc.metadata.recipient && (
                    <div>
                      ...
                      <p className="text-xs text-gray-900">
                        <TruncatedHash value={selectedDoc.metadata.recipient} />
                      </p>
                    </div>
                  )}
                  {selectedDoc.metadata.expiryDate && (
                    <div>
                      ...
                      <p className="text-gray-900">
                        {selectedDoc.metadata.expiryDate.toLocaleDateString()}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div>
              <p className="text-xs text-gray-500 mb-2 font-semibold uppercase tracking-wide">
                Co-Signers ({selectedDoc.coSigners.length})
              </p>
            ...
            <div className="flex items-center gap-3 pt-2 border-t">
              <div
                className={`flex items-center gap-2 px-3 py-2 rounded-lg ${selectedDoc.coSignQualified ? "bg-teal-50 text-teal-800" : "bg-yellow-50 text-yellow-800"}`}
              >
                {selectedDoc.coSignQualified ? (
                  <CheckCircle size={16} />
                ) : (
