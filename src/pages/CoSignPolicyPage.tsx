  ...
  const handleCreate = () => {
    const policy: CoSignPolicy = {
      ...
      requiredRoles: createForm.requiredRoles
        ? createForm.requiredRoles.split(",").map((r) => r.trim())
        : [],
      whitelistedOperators: createForm.whitelistedOperators
        ? createForm.whitelistedOperators.split(",").map((a) => a.trim())
        : [],
    };
    setConfirm({
      open: true,
      ...
      onConfirm: async () => {
        ...
        try {
          await blockchainService.setCoSignPolicy(
            policy,
            session?.address || "",
          );
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Configure multi-signature requirements per document type
          </p>
        </div>
        {canManage && (
          <button
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="px-5 py-8 text-center text-gray-400 text-sm">
            No co-sign policies configured. Create one to require
            multi-signature verification for documents.
          </div>
        ) : (
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-bold text-gray-900">
                          {docTypeLabel[policy.docType] || policy.docType}
                        </h3>
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-semibold ${policy.enabled ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}
                        >
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        ...
                        {policy.requiredRoles.length > 0 &&
                          ` | Roles: ${policy.requiredRoles.join(", ")}`}
                        {policy.whitelistedOperators.length > 0 &&
                          ` | ${policy.whitelistedOperators.length} whitelisted`}
                      </p>
                    </div>
                  </div>
                  {canManage && (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        style={{
                          color: policy.enabled ? "#b45309" : "#16a34a",
                          borderColor: policy.enabled ? "#fbbf24" : "#86efac",
                        }}
                      >
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              <div>
                ...
                <AppSelect
                  ...
                  onChange={(value) =>
                    setCreateForm({ ...createForm, docType: value })
                  }
                  data={[
              ...
              <div>
                ...
                <input
                  ...
                  min={1}
                  max={10}
                  value={createForm.minSigners}
                  onChange={(e) =>
                    setCreateForm({
                      ...createForm,
                      minSigners: parseInt(e.target.value) || 2,
                    })
                  }
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">
                  Whitelisted Operators (comma-separated addresses)
                </label>
                <input
                  type="text"
                  value={createForm.whitelistedOperators}
                  onChange={(e) =>
                    setCreateForm({
                      ...createForm,
                      whitelistedOperators: e.target.value,
                    })
                  }
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${createForm.enabled ? "translate-x-5" : ""}`}
                  />
                </button>
